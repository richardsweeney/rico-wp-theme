    ...
    return this.each(function () {
      ...
      var methods = {
        ...
        open: function(link) {
          ...
          $next.text(translations.next);
          $prev.text(translations.prev);
          $pause.text(translations.pause);
          ...
          if (!$('body').hasClass('rps-lightbox-active')) {
            ...
            $content.css({width: 100, height: 100, 'margin-top': ((winHeight - 100) / 2) + scrollTop});
          } else {
          ...
          $image.load(function () {
            ...
            var imgHeight = $image.naturalHeight() + 20,
                imgWidth = $image.naturalWidth() + 20;

            if (winWidth < imgWidth) {
              imgHeight = $image.height() + 20;
            }
            top = ((winHeight - imgHeight) / 2) + scrollTop;
            if(title !== '') {
              top = top - 10; // 20 px padding is added to the title div
            }
            ...
            $content.css({'margin-top': top, height: imgHeight, 'width': imgWidth}).append($close).append($pause);
        ...
        pause: function() {
          window.clearTimeout(methods.timeoutActive);
          var nextElement = methods.getElement('next');
          $(this).addClass('paused').text(translations.start).bind('click', function() {
            methods.open(nextElement);
            return false;
          });
          return false;
